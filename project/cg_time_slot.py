...
class TimeSlot(app_commands.Group):
    def __init__(self):
        super().__init__(name="timeslot", description="Kingshot Player Time Slot Tools")
    ...
    #       - priority:       integer
    #   validated_ind:        defaults to 1 (true), the scheduling admin can set this to 0 (false) and ask the player to confirm their time slot is accruate.
    #   create_date_time:     set to the current timestamp
    #   update_date_time:
    #   create_account_id:    set to the account_id of the person executing the command
    #   update_account_id:
    @app_commands.command(name="add", description="Add a Time Slot for a Kingshot Player")
    async def add(
            self,
    ...
    @app_commands.command(name="edit", description="Edit a Time Slot for a Kingshot Player")
    async def edit(
            self,
            interaction: discord.Interaction,
            player_id: int | None = None,
    ...
    @app_commands.command(name="remove", description="Removes a Time Slot for a Kingshot Player")
    async def remove(
            self,
            interaction: discord.Interaction,
            player_id: int | None = None,
    ...
    @app_commands.command(name="list", description="List Time Slots for a Kingshot Player")
    async def list(
            self,
            interaction: discord.Interaction,
            player_id: int | None = None,
    ...
    @app_commands.command(name="show", description="Show a Time Slot for a Kingshot Player")
    async def show(
            self,
            interaction: discord.Interaction,
            player_id: int | None = None,
