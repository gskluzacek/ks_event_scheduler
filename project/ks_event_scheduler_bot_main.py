...
from database import (
    ...
    get_players,
    create_admin,
    get_accounts, get_players_for_account,
)
...
async def player_id_with_recent_autocomplete(
    ...
    if cvl_at_least_admin.check(users_roles):
        ...
        players = await get_players(recent_player_ids)
    else:
        players = await get_players_for_account(session.get(interaction.user.id, "account_id"))
    ...
    return [
        ...
        for kingshot_name, player_id in players
    ]
...
class Player(app_commands.Group):
    ...
    async def show(
        ...
        # no bot-mode restriction on read only commands
        chk_ok, chk_msg = cc_bot_mode_any.command_check(interaction.user.id)
        if not chk_ok:
            await interaction.response.send_message(chk_msg, ephemeral=True)
            return
...
```""",
    ...
    async def add(
        ...
        chk_ok, chk_msg = cc_player_add_bot_mode_normal.command_check(interaction.user.id)
        if not chk_ok:
            await interaction.response.send_message(chk_msg, ephemeral=True)
            return
...
class Admin(app_commands.Group):
    ...
    # /admin bot_mode
    @app_commands.command(name="bot_mode", description="Change the bot mode")
    async def bot_mode(
            self,
            interaction: discord.Interaction,
            new_mode_str: str
    ):
        chk_ok, chk_msg = cc_admin_bot_mode_at_least_admin.command_check(interaction.user.id)
        if not chk_ok:
            await interaction.response.send_message(chk_msg, ephemeral=True)
            return

        new_mode = BotMode.from_str(new_mode_str)
        if new_mode is None:
            await interaction.response.send_message(
                f"❌ Invalid bot mode: {new_mode_str}. Valid modes are: {[m.value for m in (BotMode.NORMAL, BotMode.MAINTENANCE, BotMode.DOWN)]}", ephemeral=True
            )
            return

        curr_mode = BotMode.from_str(app_state.get("bot_mode"))
        if curr_mode == BotMode.NORMAL and new_mode not in (BotMode.MAINTENANCE, BotMode.DOWN):
            await interaction.response.send_message(
                f"❌ Cannot change bot mode from {curr_mode.value} to {new_mode.value}. "
                f"Valid modes are: {[m.value for m in (BotMode.MAINTENANCE, BotMode.DOWN)]}", ephemeral=True
            )
            return
        elif curr_mode == BotMode.MAINTENANCE and new_mode not in (BotMode.NORMAL, BotMode.DOWN):
            await interaction.response.send_message(
                f"❌ Cannot change bot mode from {curr_mode.value} to {new_mode.value}. "
                f"Valid modes are: {[m.value for m in (BotMode.NORMAL, BotMode.DOWN)]}", ephemeral=True
            )
            return
        elif curr_mode == BotMode.DOWN and new_mode not in (BotMode.NORMAL, BotMode.MAINTENANCE):
            await interaction.response.send_message(
                f"❌ Cannot change bot mode from {curr_mode.value} to {new_mode.value}. "
                f"Valid modes are: {[m.value for m in (BotMode.NORMAL, BotMode.MAINTENANCE)]}", ephemeral=True
            )
            return
        elif curr_mode == BotMode.UNKNOWN:
            await interaction.response.send_message(
                f"❌ Invalid current bot mode: {curr_mode.value}.", ephemeral=True
            )
            return
